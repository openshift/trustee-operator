...
func TestNewKbsNetworkPolicies_AllowIngress(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The router is hostNetwork: true; an empty podSelector must accompany the
			// namespaceSelector so the peer matches hostNetwork pods on OVN-Kubernetes.
			if ps := np.Spec.Ingress[1].From[0].PodSelector; ps == nil || len(ps.MatchLabels) != 0 || len(ps.MatchExpressions) != 0 {
				t.Errorf("router ingress peer must set an empty podSelector to match hostNetwork pods, got %v", ps)
			}
